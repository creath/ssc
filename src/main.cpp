...
uint256 hashGenesisBlock("0x0000040d6ac7bab6c750e57c921b72caadad856fbb989985e0c81a246320c8cf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SSCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SSCoin Signed Message:\n";
...
{
    // SSCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 2200000000 * COIN;
	
    nSubsidy >>= (nHeight / 12500000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60;
static const int64 nTargetSpacing = 64;
...
{
	static const int64	BlocksTargetSpacing			= 64;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * (64.0/60.0) * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * (64.0/60.0) * 2.8;
...
{
    // SSCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "SSCoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb9, 0xde }; // SSCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SSCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SSCoinMiner : ProcessBlock, block not accepted");
    }
